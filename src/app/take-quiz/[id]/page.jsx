...
export default function TakeQuiz({ params }) {
  ...
  const fetchQuiz = useCallback(async (token) => {
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(
          `/quiz-result/${params.id}?score=${result.score}&total=${result.totalQuestions}`
        );
      } else {
        const errorData = await response.json();
        setError(
          `Failed to submit quiz: ${errorData.message || response.statusText}`
        );
      }
    } catch (error) {
      console.error('Error submitting quiz:', error);
      setError(`An error occurred while submitting the quiz: ${error.message}`);
    }
  };
